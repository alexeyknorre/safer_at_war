...
calculate_shooting_counts <- function(city_name,
  ...
  age_group_mapping <- data.frame(age_group = c(0,18,25,45,65),
  ...
  acs_zcta <- readRDS(paste0("data/zcta_acs/",city_name,
                                               "_acs.rds"))
  
  # Open previously created shooting victims
  shootings <- fread(file.path("data",
                           "shootings_csv",
  ...
  shootings <- shootings %>%
    ...
    if (city_name == "New York") {
      ...
      # Aggregate population counts by new age groups
      acs_zcta <- acs_zcta %>% mutate(age_group = cut(as.numeric(age_group),
                                      breaks = c(city_age_group_breaks,120),
                                      labels = c(city_age_group_labels),
                                      right = F)) %>% 
        mutate(age_group = as.character(age_group)) %>% 
    ...
    shootings_aggregated <- shootings_projected %>%
      ...
      summarize(lethal_shootings_count = sum(shooting_type == "lethal"),
    ...
    shooting_counts <- left_join(acs_zcta, shootings_aggregated,
      ...
      mutate(lethal_shootings_count = ifelse(is.na(lethal_shootings_count),
                                             0,
  ...
  shooting_counts <- shooting_counts %>% 
    ...
    select(city, geo_label, age_group, sex, race, population,
