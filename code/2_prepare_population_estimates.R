...
prepare_population_estimates <- function(){
  ...
  get_acs_population_estimates_by_age <- function(city_name,
    ...
    # Save ZCTA geometry
    acs_geometry <- acs %>% mutate(geo_label = NAME) %>% select(geo_label)
    ...
    acs_race_populations <- left_join(acs_totals, acs_whites, by = c("geo_label",
                                                                     "age_group",
                                                                     "sex")) %>% 
      mutate(population_POC = population_total - population_White) %>% 
      select(-one_of(c("population_total"))) %>% 
      pivot_longer(cols = population_White:population_POC,
